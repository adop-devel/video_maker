...
def make_article_video(data):
    ...
    # 타이틀 옆 로코 그기 정의
    logoImg = cv2.imread('%s/logo/finalLike.png' % VIDEO_MAKER_PATH)
    ...
    # 비디오 설정
    pathOut = '%s/video/news/%s.mp4' % (VIDEO_MAKER_PATH, firstTitle)
    fps = 15
    frame_array = []
    ...
    out.release()
    # 비디오 폴더 초기화
    video_path = "%s/video" % VIDEO_MAKER_PATH
    if os.path.isfile(video_path):
        # 이게 이미비디오 만들어진게 으면 삭제하라는거건데..
        shutil.rmtree(video_path)  # 디렉토리 + 안에 있는 파일도 삭제
        os.mkdir(video_path)
    # 이부분이 비디오 인코딩 하면서 용량줄이는 건데.. 왜 안되는지 지금을 파익이 좀 어렵네요. 일단 이 위까지해서 코드 파악해보시고 , 리펙토링 진행해주세요
